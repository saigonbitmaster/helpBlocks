  ...
  const CaseVoteButton = (props) => {
    ...
    React.useEffect(() => {
      dataProvider
        .customMethod(
          "customapis/vote",
          { filter: { queryType: "unlock", caseId: record._id } },
          "GET"
        )
    ...
    const handleClick = () => {
      dataProvider
        .customMethod("customapis/unlockvote", { data: voteBody }, "POST")
  ...
  return (
    ...
    >
      <Datagrid bulkActionButtons={false}>
        ...
        <FunctionField
          label="Number of votes to unlock fund"
          render={(record) => (
            <>
              <Chip
                icon={<ThumbDownAltIcon />}
                label={
                  record.unlockVotes.filter((item) => item.vote === false).length
                }
                color="warning"
              />
              <Chip
                ...
                icon={<ThumbUpAltIcon />}
                label={
                  record.unlockVotes.filter((item) => item.vote === true).length
                }
              />
            </>
          )}
        ...
        <CaseVoteButton label="Vote to unlock" />
